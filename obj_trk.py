...
video = cv2.VideoCapture("sunrise.mp4")
#load tracker 
...
def goal_track(img,bbox):
    ...
    for i in range(len(xs)-1):
        cv2.circle(img,(xs[i],ys[i]),2,(0,0,755),6)
...
def goal_track(img, bbox):
    ...
    cv2.circle(img,(c1,c2),2,(0,0,755),6)

    cv2.circle(img,(int(p1),int(p2)),2,(0,755,0),6)
    ...
    for i in range(len(xs)-1):
        cv2.circle(img,(xs[i],ys[i]),2,(0,0,755),5)
